...
const returnPrintFn = (obj: object): Function => {
...
function createUser(userInfo: { name: string; isPaid: boolean }): {
  isLoggedIn: boolean;
  userName: string;
} {
...
const getDetails = (userInfo: {
  name: string;
  email: string;
}): { enrolled: boolean; memberSince: Date } => {
  console.log("User Info: ", userInfo);
  return {
    enrolled: true,
...
// This is OK!
console.log(
  getDetails({ name: "Deathslayer", email: "deathslayer@gmail.com" })
);
...
// - Methods in Objects - 
// 1. Simple Method with return type - 
type Fish = {
  name: string;
  type: string;
  swim(a: number): string;
};

// 2. Method with arrow function with return type - 
type Bird = {
  name: string;
  type: string;
  fly: (b: string) => void;
...
// - The "in" operator for narrowing types in case of objects, types, or interfaces -
interface User {
  name: string;
  email: string;
}
